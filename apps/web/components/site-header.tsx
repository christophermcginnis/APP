...
export function SiteHeader() {
  ...
  const acknowledgeNotifications = trpc.profile.acknowledgeNotifications.useMutation();
  ...
  const wasNotificationsOpenRef = useRef(notificationsOpen);
  ...
  useEffect(() => {
    if (wasNotificationsOpenRef.current && !notificationsOpen) {
  ...
  const acknowledgementInput = user.isAuthenticated
    ? {
        userId: user.id,
  ...
  function handleNotificationsToggle() {
    ...
    if (willOpen) {
      void (async () => {
        ...
        const latestNotifications = result.data ?? notifications;

        if ((latestNotifications?.length ?? 0) > 0 && acknowledgementInput) {
          try {
            await acknowledgeNotifications.mutateAsync(acknowledgementInput);
          } catch {
            // Swallow acknowledgement errors to avoid blocking the dropdown.
          }
        }
      })();
    }
